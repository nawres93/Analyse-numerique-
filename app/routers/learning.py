...
@router.get("/students/me/progress/{module_id}", response_model=ModuleProgressPublic)
async def get_my_module_progress(
    module_id: str,
    current_user: dict = Depends(get_current_user),
):
    return await progress_service.get_module_progress(
        str(current_user["_id"]), module_id
    )
